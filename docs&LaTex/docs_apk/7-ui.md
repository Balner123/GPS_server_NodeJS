...
- Interní konzole (`ConsoleLogger`) je dostupná pouze ve vývojovém nebo debug režimu; slouží pro rychlou diagnostiku (limit záznamů např. 100). V produkci by konzole měla být skrytá.

## Přístupnost a notifikace

- Notifikace služby musí obsahovat srozumitelný text a kontextové akce pro uživatele.
- UI by mělo respektovat systémové zásady pro pozadí a šetření energie (neprovokovat zbytečná volání UI při background operacích).
