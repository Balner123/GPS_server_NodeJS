...
\begin{itemize}
  ...
  \setlength\parsep{0pt}
    ...
    \item \texttt{Device}: Reprezentuje hardwarové jednotky. Kromě identifikátoru a~vazby na~uživatele uchovává konfiguraci synchronizovanou s~HW: \texttt{interval\_gps} (četnost snímání), \\ \texttt{interval\_send} (četnost odesílání), \texttt{mode} (režim simple/batch) a~\texttt{geofence} (definice bezpečnostní zóny ve~formátu GeoJSON).
...
\subsection{Rozhraní API a komunikace}
\label{sec:server_api}
V~rámci optimalizace backendu došlo ke~sjednocení registračních procesů a~standardizaci HTTP metod. 
Pro~registraci všech typů zařízení (HW i~APK) nyní slouží centrální endpoint \\ \texttt{/api/devices/register},
kde se typ klienta rozlišuje parametrem \texttt{client\_type}.

\subsubsection{API - HW tracker (ESP32)}
HW klient se v~tuto chvíly při~komunikaci autentizuje pouze pomocí \texttt{device\_id} (získaného z~MAC adresy). Toto ID je zaregistrováno, pokud uživatel vloží správné uživatelské údaje v~registrační části OTA režimu. HW klient nevyužívá session ani cookies.
...
\subsubsection{Zpracování a agregace GPS dat}
\label{subsec:server_gps_processing}
Jedním z~úkolů serveru je efektivní zpracování velkého množství dat o~poloze. Aby se předešlo zahlcení mapy redundantními body, implementuje server tzv. \textbf{"On-Read" agregaci}.
Původní data jsou v~databázi uložena v~surové podobě, což zaručuje jejich integritu. Ke~shlukování dochází až v~momentě Fetchovaní dat pro~zobrazení na~mapě.

Algoritmus \texttt{clusterLocations} prochází časově seřazené záznamy a~sdružuje body, které jsou si vzájemně blízké (vzdálenost menší než X metrů). 
Výsledkem je pole, které obsahuje jak jednotlivé body (v~pohybu), tak "clustery" (shluky), které reprezentují stání na~místě v~určitém časovém intervalu.
...
Hesla uživatelů nejsou nikdy ukládána v~otevřené podobě. Při registraci je na heslo aplikována hashovací funkce \texttt{bcrypt}. Při přihlášení se zadané heslo zahashuje a~porovná s~uloženým hashem. To chrání uživatele i~v~případě úniku databáze.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.2\textwidth]{../docs_server/schemas/Use_case_diagram.png}
    \caption{Use-case diagram systému LOTR}
    \label{fig:use_case}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=1.05\textwidth]{../docs_server/schemas/sequence_diagram_tracking.png}
