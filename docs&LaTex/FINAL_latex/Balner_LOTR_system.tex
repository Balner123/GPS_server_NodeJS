...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\noindent Děkuji těm, jimž poděkováno zatím nebylo. Také děkuji Současné době za to, že jsou již LLM na takové urovni, díky čemuž kódování tohoto projektu bylo možné i za sníženého pracovního nasazení a kognitivního vypětí.\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	zajišťuje sběr telemetrických dat, správu uživatelů a poskytuje REST API pro komunikaci s koncovými zařízeními. Doplňkovou součástí systému je také nativní aplikace pro OS Android vyvinutá v jazyce Kotlin, která slouží jako softwarová (a testovací) náhrada hardwarového trackeru.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%		
	...
	Jako méně důležitou, spíše doplňkovou, ale přesto užitečnou součást systému jsem pak viděl výtvoření mobilní aplikace pro systém Android. Tato aplikace však nemá takové důležitosti jako ostatní komponenty, jelikož byla koncipována jako testování API a pro neznalost Kotlinu většina jejího kódu byla generována LLM (chcete-li "AI", konkrétně Gemini).
	Tato dokumentace ji, ale i přesto popisuje a rozvádí i její kód, protože vygenerovaný kód je kvalitní alespoň co do úrovně využitých principů a zásad. 
...
\begin{enumerate}
  ...
  \setlength\parsep{0pt}
	...
	\item Popisuje postup vývoje aplikace a její architekturu; jak zmíněno výše, jde o analýzu vzniklou reverzním inženýrstvím, ne přímou tvorbou.
\end{enumerate}
